...
# PHP 7.4 不在当前主源, 固定使用 Alpine v3.15 归档源
APK_REPO="--repository https://dl-cdn.alpinelinux.org/alpine/v3.15/main --repository https://dl-cdn.alpinelinux.org/alpine/v3.15/community"
...
install() {
    ...
    (
        cd "$dl_dir"
        apk fetch --recursive $APK_REPO "$PKG" "$PKG-fpm"
    ) || {
        echo "错误: 未找到 $PKG 相关软件包" >&2
    ...
    if [ -f "$ext_dir/usr/bin/$PKG" ]; then
        ...
        ln -sf "$PHP_BIN" "/usr/bin/$PKG"
    else
        echo "错误: 未找到 $PKG 二进制" >&2
        rm -rf "$dl_dir" "$ext_dir"
        exit 1
    fi

    # 拷贝依赖库: 软链解引用成真实文件(-fL), 避免相对软链(../../lib/..)断裂
    for d in "$ext_dir/lib" "$ext_dir/usr/lib"; do
        if [ -d "$d" ]; then
            find "$d" \( -type f -o -type l \) -name '*.so*' -exec cp -fL {} "$LIB_DIR/" \; 2>/dev/null || true
            find "$d" -maxdepth 1 -type f ! -name '*.so*' -exec cp -f {} "$LIB_DIR/" \; 2>/dev/null || true
        fi
    done

    if [ -d "$ext_dir/usr/lib/$PKG" ]; then
        cp -r "$ext_dir/usr/lib/$PKG" "$LIB_DIR/" 2>/dev/null || true
    fi

    if [ -d "$ext_dir/etc/$PKG" ]; then
        cp -r "$ext_dir/etc/$PKG/." "$CONF_DIR/"
    fi

    if [ -f "$CONF_DIR/php.ini" ]; then
        echo "extension_dir = $LIB_DIR/$PKG/modules" >> "$CONF_DIR/php.ini"
    fi

    ghproxy_val=$(grep '^GHPROXY=' /www/server/panel/.env 2>/dev/null | cut -d= -f2-)
    GH_PROXY=""
    [ -n "$ghproxy_val" ] && [ "$ghproxy_val" != "false" ] && GH_PROXY="$ghproxy_val"
    PHP_RAW="${GH_PROXY}https://raw.githubusercontent.com/svier0/alpanel-plugins/master/plugins/php74"

    # PHP 7.4 依赖 openssl1.1/libcrypto1.1(v3.15 是 openssl1.1, 系统 v3.21 是 openssl3), 从 v3.15 拉取放入 LIB_DIR
    if ! ls "$LIB_DIR"/libssl.so.1.1 >/dev/null 2>&1; then
        ssl_dir=$(mktemp -d)
        (
            cd "$ssl_dir"
            apk fetch --recursive $APK_REPO libssl1.1 libcrypto1.1 >/dev/null 2>&1
        ) || true
...
PHINIT
    ...
    # json 为独立扩展包(php7 的 --enable-json=shared), get_fpm_status 与 PHP 站点依赖, 默认必装
    install_one_ext "json" >/dev/null 2>&1 \
        || echo "  [warn] 扩展 json 安装失败"
...
$sock = getenv("FPM_SOCK") ?: "/tmp/php-cgi-74.sock";
...
$p = [
  ...
  "REQUEST_METHOD"    => "GET",
  "REQUEST_URI"       => "/phpfpm_74_status",
  "SCRIPT_NAME"       => "/phpfpm_74_status",
  "SCRIPT_FILENAME"   => "/phpfpm_74_status",
  "QUERY_STRING"      => $query,
  "SERVER_SOFTWARE"   => "alp",
...
PHPEOF
    ...
    FPM_SOCK="$sock" FPM_QS="json" "$PHP_BIN" -c "$INI_FILE" "$script" 2>/dev/null
    code=$?
    rm -f "$script"
...
get_local_exts() {
    ...
    builtin=$( "$PHP_BIN" -n -m 2>/dev/null \
        | sed -n '/\[PHP Modules\]/,/^$/p' \
        | grep -vE '^\[|^$' )
    so=$(ls "$LIB_DIR/$PKG/modules"/*.so 2>/dev/null | xargs -n1 basename 2>/dev/null | sed 's/\.so$//' || true)
...
build_ext_cache() {
    ...
    apk search $APK_REPO -v 'php7-*' 2>/dev/null > "$tmp" || { rm -f "$tmp"; echo '{"error":"apk search 失败"}'; return 1; }
    awk -F ' - ' 'NF>=2 && $1 ~ /^php7-/ {
        pkg=$1; sub(/^php7-/,"",pkg); sub(/-[0-9][0-9a-zA-Z._-]*$/,"",pkg)
        if (pkg !~ /^[a-zA-Z]/) next
...
install_one_ext() {
    ...
    dl_dir=$(mktemp -d)
    ext_dir=$(mktemp -d)
    ( cd "$dl_dir" && apk fetch --recursive $APK_REPO "$PKG-$pkg" >/dev/null 2>&1 ) || {
        rm -rf "$dl_dir" "$ext_dir"
