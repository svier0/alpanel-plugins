...
install() {
    ...
    PHP_RAW="${GH_PROXY}https://raw.githubusercontent.com/svier0/alpanel-plugins/master/plugins/php84"
...
$sock = getenv("FPM_SOCK") ?: "/tmp/php-cgi-84.sock";
...
$p = [
  ...
  "REQUEST_METHOD"    => "GET",
  "REQUEST_URI"       => "/phpfpm_84_status",
  "SCRIPT_NAME"       => "/phpfpm_84_status",
  "SCRIPT_FILENAME"   => "/phpfpm_84_status",
  "QUERY_STRING"      => $query,
  "SERVER_SOFTWARE"   => "alp",
...
build_ext_cache() {
    ...
    apk search -v 'php84-*' 2>/dev/null > "$tmp" || { rm -f "$tmp"; echo '{"error":"apk search 失败"}'; return 1; }
    awk -F ' - ' 'NF>=2 && $1 ~ /^php84-/ {
        pkg=$1; sub(/^php84-/,"",pkg); sub(/-[0-9][0-9a-zA-Z._-]*$/,"",pkg)
        if (pkg !~ /^[a-zA-Z]/) next
