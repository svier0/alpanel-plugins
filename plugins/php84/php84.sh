...
install() {
    ...
    command -v apk >/dev/null 2>&1 || { echo "閿欒: 浠呮敮鎸?Alpine Linux" >&2; exit 1; }
    ...
    (
        ...
        echo "閿欒: 鏈壘鍒?php$VER 鐩稿叧杞欢鍖? >&2
...
status() {
    if [ -f "$PIDFILE" ]; then
        ...
        if kill -0 "$PID" 2>/dev/null; then
            echo "php__VER__-fpm 杩愯涓?(pid $PID)"
            return 0
        fi
    fi
    echo "php__VER__-fpm 鏈繍琛?
    return 1
}
...
if [ -z "${RC_SVCNAME:-}" ]; then
    case "${1:-}" in
        ...
        status)  status ;;
        *)       echo "鐢ㄦ硶: $0 {start|stop|restart|status}" >&2; exit 1 ;;
    esac
fi
PHINIT
